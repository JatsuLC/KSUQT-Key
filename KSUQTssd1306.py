import time, os, usb_hid, board, digitalio, displayio, terminalio, adafruit_displayio_ssd1306
...
def execCode(scriptList):
    def update_display(line):
        if updateDisplay:
            updating_label.text = "\n".join(wrap_text_to_pixels(scriptName + "\n" + " ".join(line), 128, terminalio.FONT))
...
###I2C Assignment###
screen = board.STEMMA_I2C()
displayio.release_displays()
display_bus = displayio.I2CDisplay(screen, device_address=0x3c)
...
updating_label = label.Label(
    font=terminalio.FONT, text="\n".join(wrap_text_to_pixels("", 128, terminalio.FONT))
)

#Label positioning
updating_label.anchor_point = (0, 0)
updating_label.anchored_position = (0, 0)
...
###OLED UPDATES WHILE EXECUTING###
updateDisplay = True

###MAIN LOOP FUNCTIONS###
def update_display(status):
    text = f"{'Running' if status == 'running' else 'Ready to run...'}\n{scriptName}"
    updating_label.text = "\n".join(wrap_text_to_pixels(text, 128, terminalio.FONT))
